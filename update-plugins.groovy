...
dockerImages.each{ dockerImage -> 
	def jenkinsVersion = dockerImage.readLines().find{ it.startsWith('FROM') }.split(':')[1]
	jenkinsVersion = jenkinsVersion.replaceAll("-slim", "")
	def plugins = [:]
	def pluginsText = new File(dockerImage.parent, 'plugins.txt')
	pluginsText.readLines().each{ plugins.put( it.split(':')[0] , it.split(':')[1] ) }
